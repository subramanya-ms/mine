...
public class T2599Class extends DriverScript{
	...
	public synchronized void setup(@Optional("opt platform") String platform ,@Optional("opt browser name") String browserName ,@Optional("opt remoteurl") String remoteurl) throws IOException{
		...
		System.setProperty("webdriver.chrome.driver",workingDir+"/seleniumjars/Anitha/chromedriver.exe");
	...
	@Test
	@Parameters({"platform" , "browserName" , "remoteurl"}) 
	public synchronized void validate_T2599Page_Report(@Optional("opt platform") String platform ,@Optional("opt browser name") String browserName ,@Optional("opt remoteurl") String remoteurl) throws IOException, AWTException{
		...
		strClassName = "Financial Reports";
...
//		System.out.println("Test Case  with Thread Id:- "+ Thread.currentThread().getId());
		...
		page.navigateToT2599Page(threadID,tempResultList ,pathLoc)
		...
		ReusableComponents.reportInfo(threadID,tempResultList,testMethod , "Test case : Validating that user can directly delete reports from the Financial Reports tab", browser ,pathLoc+"\\"+TESTCASENAME , false );
